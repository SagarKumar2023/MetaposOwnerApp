import 'dart:convert';
import 'dart:developer';
import 'package:http/http.dart' as http;
import 'package:get/get.dart';
import 'package:metapos_owner/controller_model/model/report_models/staff_list_model.dart';
import 'package:metapos_owner/utils/app_url.dart';

class StaffListController extends GetxController{

  List<Data> _data = [];
  List<Data> get data => _data;

  bool _isLoading = false;
  bool get isLoading => _isLoading;

  Future<void> getStaffListData(Map<String,dynamic> body) async {

    _isLoading = true;
    update();

    String to = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..HMl_kyES_m2mqKHA844DgKLQzIu7nj04QlPKO5jrsoEcT8XMGYUngu89RCiD2ZJnPWfGBVV1eBFA2tkJGqbRH5qsj-NMdFGQO5MrwIHXkNllhaMN6MvjS4j850NEXzNEmPpV3rRkcLbI4uq-jMTBVkdrsgt4NZcLID4tb_bkR533J9H69TiS_YyMyqXrQ-gMR3uUExinqABCH1nulGNFC6T0pUkBnvCkXBy_LJTDbnN6boizD4Xv9Gjjr8JywtJWiuxdLBFz64211toDBQgl7D4peg2fW3f2H6ajMX7C5LJvRC73-1zqyhJGItG_v-xFovEIJGIJRrMw0-GtFD6AgmAs2rWwOm53fgB-XCJdC4883ek7cniZ38NyueZb0gqA1yPZInVDCDnciD6qsnjOuhdR-dQLKo-CEolDmYyZGPcHwMT1-mc-47SHrKX3B8dQay7XdblWgdOPF5oPftkTVK2llk9MpGtPtTV-qmfjwHT_hsQZhZhZPqcdWhw-aFE93Ga0pxOMK__hIJyrd1ojUh0-klsuQ6XSuXJptRhHcUBLi_c0cJ5KdQ6SvP6KSRcP-qVfcvBhN3FZXoJQImSyKOfte_j1qdV5BaHhZR6L93OQRsimYdEZPLC50qbKQ_uLc2EH-1lmn-nYbECJeXDsAx24G74Q1U87X7sadgcCr38";

    var headers ={
      "Authorization" : "Bearer $to",
    };
    
    try{
      var response = await http.post(Uri.parse(AppUrl.staffAttendanceList),
      body: body,
        headers: headers,
      );

      if(response.statusCode == 200){
        log("API URL => ${AppUrl.staffAttendanceList}");
        log("Response Body => ${response.body}");

        StaffListModel staffListModel = StaffListModel.fromJson(jsonDecode(response.body));

        if(staffListModel.error == 0){
          _data = staffListModel.data!;
          _isLoading = false;
          update();

          log("Success Staff Attendance Loaded Successfully");
          Get.snackbar("Success", "Staff Attendance Loaded Successfully");
        }else{
          log("Error: ${staffListModel.error}");
          Get.snackbar("Error", staffListModel.error.toString(),duration: Duration(seconds: 3));
          _isLoading = false;
          update();
        }
      }else {
        log("Error: ${response.statusCode}");
        Get.snackbar("Error", response.statusCode.toString(),duration: Duration(seconds: 3));
        _isLoading = false;
        update();
      }
      
    }catch (e,stack){
      log("Exception: $e");
      log("StackTrace: $stack");
      _isLoading = false;
      update();
    }finally{
      _isLoading = false;
      update();
    }

  }

}